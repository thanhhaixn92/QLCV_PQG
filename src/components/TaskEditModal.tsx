  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex justify-end" onClick={onClose}>
      <motion.div 
        ...
        className="bg-white w-full max-w-2xl md:max-w-3xl lg:max-w-4xl h-full shadow-2xl overflow-hidden flex flex-col"
        ...
        <div className="overflow-y-auto custom-scrollbar flex-1 flex flex-col">
          ...
          <div className="p-4 sm:p-8 flex-1 space-y-6">
            <div className="space-y-2">
              <label className="text-[10px] font-semibold text-slate-400 tracking-normal ml-1">Tiêu đề công việc</label>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-semibold text-slate-400 tracking-normal ml-1">Người xử lý</label>
              <input 
                type="text" 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-semibold text-slate-400 tracking-normal ml-1">Lĩnh vực chuyên môn</label>
              <select 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-semibold text-slate-400 tracking-normal ml-1">Thời hạn xử lý</label>
              <input 
                type="date" 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-semibold text-slate-400 tracking-normal ml-1">Độ ưu tiên</label>
              <select 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-semibold text-slate-400 tracking-normal ml-1">Trạng thái hiện tại</label>
              <select 
            ...
            <div className="space-y-3 pt-2">
              <label className="text-[10px] font-semibold text-slate-400 tracking-normal ml-1">Tài liệu liên quan ({editingTask.linkedDocumentIds?.length || 0})</label>
              <div className="flex flex-wrap gap-2">
                {editingTask.linkedDocumentIds?.map((docId, idx) => {
                  const doc = documents.find(d => d.id === docId);
                  if (!doc) return null;
                  return (
                    <div key={`${docId}-${idx}`} className="flex items-center gap-2 bg-[#002D56] text-white px-3 py-1.5 rounded-md text-[10px] font-semibold uppercase shadow-sm">
                ...
                <button 
                  ...
                  className="flex items-center gap-2 bg-white text-[#002D56] px-3 py-1.5 rounded-md text-[10px] font-semibold uppercase border border-dashed border-[#002D56]/30 hover:bg-[#002D56]/5 transition-all"
                >
            ...
            <div className="flex flex-col justify-center gap-2 pt-2">
              <label className="flex items-center gap-3 cursor-pointer select-none bg-slate-50 p-4 rounded-md border border-slate-100 hover:border-[#002D56]/20 transition-all">
                ...
                <div className="flex flex-col text-left">
                  <span className="text-xs font-semibold text-slate-700 tracking-tight">Chế độ kiêm nhiệm</span>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-semibold text-slate-400 tracking-normal ml-1">Mô tả chi tiết</label>
            <textarea 
              rows={4}
          ...
          <div className="pt-6 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3 w-full sm:w-auto">
              {editingTask.id && (
                <button 
                  ...
                  className="flex-1 sm:flex-none px-6 py-3 rounded-md text-[10px] font-semibold uppercase text-red-500 hover:bg-red-50 transition-all border border-red-100"
                >
                  Xóa công việc
                </button>
              )}
              <button 
                onClick={onClose}
                className="flex-1 sm:flex-none px-6 py-3 rounded-md text-[10px] font-semibold uppercase text-slate-400 hover:bg-slate-200 transition-all"
              >
                Hủy bỏ
              </button>
            </div>
            <button 
              ...
              className="w-full sm:w-auto bg-[#002D56] text-white px-10 py-4 rounded-md text-xs font-semibold tracking-normal shadow-sm hover:shadow-[#002D56]/20 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
            >
