...
function tableCellToDocxCell(cell: ArticleExportTableCell): { runs: ExportTextRun[]; header?: boolean } {
  return {
...
function mapArticleExportBlockToDocxBlocks(block: ArticleExportBlock): ExportArticleBlock[] {
  switch (block.type) {
    case "title":
...
export function mapArticleExportModelToDocxBlocks(model: ArticleExportModel): ExportArticleBlock[] {
  return model.blocks.flatMap(mapArticleExportBlockToDocxBlocks);
}

export function hasDocxExportableContent(model: ArticleExportModel): boolean {
  return mapArticleExportModelToDocxBlocks(model).length > 0;
}
